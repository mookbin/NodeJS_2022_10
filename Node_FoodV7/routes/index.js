...
/**
 * select 를 할 때
 * PK 를 기준으로 조건 조회(WHERE SELECT) 를 하면
 * 데이터는 1개 이하만 나타난다
 * 다른 칼럼을 기준으로 조건 조회를 하면
 * 데이터는 0개 이상이 나타나는데
 * 만약 1개만 나타나더라도 이 데이터는 LIST(배열, 여러 개로 인식)
 *
 * PK를 기준으로 조건 조회를 할 때는
 * Today.findOne({where: {t_seq:req.params.t_seq}}) 를 사용한다
 * 이때는 데이터가 LIST 가 아닐 수 있다
 *
 * PK 일 때 권장하는 함수
 * Today.findByPk({where: {req.params.t_seq}}) 를 사용한다
 *
 */
