document.addEventListener("DOMContentLoaded", () => {
  /**
   * 데이터가 표시된 td 를 클릭하면
   * td 있는 데이터를 input box에 표시하기
   *
   * td 에 클릭 event 를 설정하려면
   * 데이터의 개수 * td 의 개수만큼 event 를 추가 해야한다
   * 효율면에서 매우 불편한 코드가 될 것이다
   *
   * 처리할 td를 감싸고있는 대상 중에서
   * 1개만 있는 box를 찾아서 그 box 에 클릭 event 를 설정하여
  ...
  tTable?.addEventListener("click", (tag) => {
    ...
    if (target.tagName === "TD") {
      ...
      // tr이 감싸고 있는 td 들을 한꺼번에 select 하기
       ...
       *
       * 변경 처리를 하려면 반드시 데이터에 seq 가 있어야한다
       *
       */
      ...
      // Object.assign() : 객체에 어떤 값을 추가하거나 할 때 사용하는 함수

      // 데이터를 수정하려면 반드시 t_seq(PK)가 필요하다
      // 현재 화면에 보이는 input box 들에는 t_seq name 을 가진 input tag 가 없다
      // 임의로 <input name="t_seq" value="PK값" style="display:hidden;"> tag 를 만들어서
      // 화면에 form 에 추가하기
